LinkLuaModifier("modifier_item_dmg_str_hp_stat_1", "items/tier1/craft_2/item_dmg_str_hp_stat_1", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_dmg_str_hp_stat_thinker_1", "items/tier1/craft_2/item_dmg_str_hp_stat_1", LUA_MODIFIER_MOTION_NONE)

item_dmg_str_hp_stat_1 = class({})

function item_dmg_str_hp_stat_1:GetIntrinsicModifierName()
	return "modifier_item_dmg_str_hp_stat_1"
end

item_dmg_str_hp_stat_1_1 = class(item_dmg_str_hp_stat_1)
item_dmg_str_hp_stat_1_2 = class(item_dmg_str_hp_stat_1)

modifier_item_dmg_str_hp_stat_1 = class({
...
function modifier_item_dmg_str_hp_stat_1:GetModifierPreAttack_BonusDamage()
	return self:GetAbility():GetSpecialValueFor("bonus_dmg")
end

function modifier_item_dmg_str_hp_stat_1:GetModifierHealthBonus()
	return self:GetAbility():GetSpecialValueFor("bonus_hp")
end

function modifier_item_dmg_str_hp_stat_1:GetModifierBonusStats_Strength()
	return self:GetAbility():GetSpecialValueFor("bonus_stat")+self:GetAbility():GetSpecialValueFor("bonus_str")
end

function modifier_item_dmg_str_hp_stat_1:GetModifierBonusStats_Agility()
	return self:GetAbility():GetSpecialValueFor("bonus_stat")
end

function modifier_item_dmg_str_hp_stat_1:GetModifierBonusStats_Intellect()
	return self:GetAbility():GetSpecialValueFor("bonus_stat")
end

function modifier_item_dmg_str_hp_stat_1:OnCreated()
...
function modifier_item_dmg_str_hp_stat_1:GetEffectName()
	return "particles/econ/events/ti9/mjollnir_shield_ti9.vpcf"
end


function modifier_item_dmg_str_hp_stat_1:OnAttackLanded(data)
	...
	if target == caster and not attacker:IsBuilding() and not attacker:IsMagicImmune() then
		...
		if RollPercentage(trigger_chance) then
			...
			local thinker = CreateModifierThinker(caster, ability, "modifier_item_dmg_str_hp_stat_thinker_1", nil, caster:GetAbsOrigin(), caster:GetTeam(), false)
			thinker.id = thinker:entindex()
			table.insert(ability.thinkers, thinker.id, thinker)
			local thinker_modifier = thinker:FindModifierByName("modifier_item_dmg_str_hp_stat_thinker_1")
			thinker_modifier:ChainLight(caster, attacker)
		end
	end
end

modifier_item_dmg_str_hp_stat_thinker_1 = class({})

function modifier_item_dmg_str_hp_stat_thinker_1:OnCreated()
...
function modifier_item_dmg_str_hp_stat_thinker_1:OnDestroy()
	table.remove(self.ability.thinkers, self.parent:entindex())
end

function modifier_item_dmg_str_hp_stat_thinker_1:ChainLight(target1, target2)
...
--	ZapThem(caster, ability, caster, target, damage)
	...
	local particle = "particles/econ/events/ti9/maelstorm_ti9.vpcf"
